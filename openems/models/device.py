...
class Device(models.Model):
    ...
    ], 'Product type', tracking=True)
    emshardware =  fields.Selection([
    ], 'EMS Hardware', tracking=True)
    oem =  fields.Selection([
        ('openems' , 'OpenEMS'),
    ], 'OEM Branding', default='openems')
    ...
    # Verknüpfungen
    user_role_ids = fields.One2many("openems.device_user_role", "device_id", string="Roles", tracking=True)
    openems_config_update_ids = fields.One2many("openems.openemsconfigupdate", "device_id", string='OpenEMS Config Updates')
    setup_protocol_ids = fields.One2many('openems.setup_protocol', 'device_id', 'Setup Protocols')
...
class DeviceUserRole(models.Model):
	...
	_sql_constraints = [
		('device_user_uniq', 'unique(device_id, user_id)', 'User already exists for this device.'),
	]
